...
fn load_benchmark_specs() -> Vec<BenchmarkSpec> {
    let possible_paths = vec![
        PathBuf::from("benches/benchmarks_dsiv_b32.jsonl"),
        PathBuf::from("oboron/benches/benchmarks_dsiv_b32.jsonl"),
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("benches/benchmarks_dsiv_b32.jsonl"),
    ];

    for path in &possible_paths {
        if path.exists() {
            eprintln!("Found dsiv.b32 benchmarks at: {:?}", path);
            ...
            let specs: Vec<BenchmarkSpec> = data
                ...
                .collect();
            eprintln!("Loaded {} dsiv.b32 benchmark specifications", specs.len());
            return specs;
        }
    }

    eprintln!("Warning: benchmarks_dsiv_b32.jsonl not found");
    vec![]
}

fn run_dsiv_b32_benchmarks(c: &mut Criterion) {
    let specs = load_benchmark_specs();

    if specs.is_empty() {
        eprintln!("No dsiv.b32 specs loaded");
        return;
    }

    // Create ob once, OUTSIDE the timed loop
    let ob = DsivB32::new_keyless().unwrap();
    ...
    eprintln!("Registered {} dsiv.b32 benchmarks", bench_count);
}

criterion_group!(benches, run_dsiv_b32_benchmarks);
