...
fn load_benchmark_specs() -> Vec<BenchmarkSpec> {
    let possible_paths = vec![
        PathBuf::from("benches/benchmarks_dgcmsiv_b64.jsonl"),
        PathBuf::from("oboron/benches/benchmarks_dgcmsiv_b64.jsonl"),
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("benches/benchmarks_dgcmsiv_b64.jsonl"),
    ];

    for path in &possible_paths {
        if path.exists() {
            eprintln!("Found dgcmsiv.b64 benchmarks at: {:?}", path);
    ...
    eprintln!("Warning: benchmarks_dgcmsiv_b64.jsonl not found");
    vec![]
}

fn run_dgcmsiv_b64_benchmarks(c: &mut Criterion) {
    let specs = load_benchmark_specs();

    if specs.is_empty() {
        eprintln!("No dgcmsiv.b64 specs loaded");
        return;
    }

    // Create ob once, OUTSIDE the timed loop
    let ob = DgcmsivB64::new_keyless().unwrap();
    ...
    eprintln!("Registered {} dgcmsiv.b64 benchmarks", bench_count);
}

criterion_group!(benches, run_dgcmsiv_b64_benchmarks);
