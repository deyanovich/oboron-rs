...
fn load_benchmark_specs() -> Vec<BenchmarkSpec> {
    let possible_paths = vec![
        PathBuf::from("benches/benchmarks_dsiv_hex.jsonl"),
        PathBuf::from("oboron/benches/benchmarks_dsiv_hex.jsonl"),
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("benches/benchmarks_dsiv_hex.jsonl"),
    ];

    for path in &possible_paths {
        if path.exists() {
            eprintln!("Found dsiv.hex benchmarks at: {:?}", path);
            ...
            let specs: Vec<BenchmarkSpec> = data
                ...
                .collect();
            eprintln!("Loaded {} dsiv.hex benchmark specifications", specs.len());
            return specs;
        }
    }

    eprintln!("Warning: benchmarks_dsiv_hex.jsonl not found");
    vec![]
}

fn run_dsiv_hex_benchmarks(c: &mut Criterion) {
    let specs = load_benchmark_specs();

    if specs.is_empty() {
        eprintln!("No dsiv.hex specs loaded");
        return;
    }

    // Create ob once, OUTSIDE the timed loop
    let ob = DsivHex::new_keyless().unwrap();
    ...
    eprintln!("Registered {} dsiv.hex benchmarks", bench_count);
}

criterion_group!(benches, run_dsiv_hex_benchmarks);
